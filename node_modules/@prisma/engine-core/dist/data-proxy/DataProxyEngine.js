...
class DataProxyEngine extends import_Engine.Engine {
  constructor(config) {
    ...
    if ((_e = this.config.previewFeatures) == null ? void 0 : _e.includes("tracing")) {
      throw new import_NotImplementedYetError.NotImplementedYetError("Tracing is not yet supported for Data Proxy", {
  ...
  resolveDatasourceURL(name, datasource) {
    ...
    if (datasource.url.fromEnvVar) {
      ...
      if (loadedEnvURL === void 0) {
        throw new import_InvalidDatasourceError.InvalidDatasourceError(`Datasource "${name}" references an environment variable "${envVar}" that is not set`, {
          clientVersion: this.clientVersion
        });
      }
      return loadedEnvURL;
    }
    throw new import_InvalidDatasourceError.InvalidDatasourceError(`Datasource "${name}" specification is invalid: both value and fromEnvVar are null`, {
      clientVersion: this.clientVersion
    });
  }
  metrics(options) {
