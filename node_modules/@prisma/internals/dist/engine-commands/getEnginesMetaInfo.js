...
async function resolveEngine(binaryName) {
  ...
  const fromEnvVarOption = (0, import_function.pipe)(pathFromEnvOption, O.map(() => envVar));
  const enginePathEither = await (0, import_function.pipe)(pathFromEnvOption, O.fold(() => (0, import_resolveBinary.safeResolveBinary)(binaryName), (_pathFromEnv) => TE.right(_pathFromEnv)))();
  const versionEither = await (0, import_function.pipe)(enginePathEither, TE.fromEither, TE.chain((enginePath) => {
