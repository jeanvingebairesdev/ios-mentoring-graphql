  ...
  for (const platform of relevantPlatforms) {
    for (const engine of ["query-engine", "introspection-engine", "migration-engine", "prisma-fmt"]) {
      for (const extension of [".gz", ".gz.sha256", ".gz.sig", ".sig", ".sha256"]) {
        const downloadUrl = (0, import_util.getDownloadUrl)(branch, commit, platform, engine, extension);
  ...
  for (const commit of commits) {
    ...
    if (!hasMissing) {
      ...
      if (missing.length !== urls.length) {
        console.log(`${import_chalk.default.blueBright("info")} The engine commit ${commit} is not yet done. We're skipping it as we're in dev. Missing urls: ${missing.length}`);
      }
    }
  }
}
