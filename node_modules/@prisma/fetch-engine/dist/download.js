...
async function download(options) {
  ...
  if (os.distro && ["nixos"].includes(os.distro)) {
    ...
    console.error(`${import_chalk.default.yellow("Warning")} Precompiled engine files are not available for ${platform}. Read more about building your own engines at https://pris.ly/d/build-engines`);
  ...
  const binaryJobs = (0, import_flatMap.flatMap)(Object.entries(opts.binaries), ([binaryName, targetFolder]) => opts.binaryTargets.map((binaryTarget) => {
...
function getCollectiveBar(options2) {
  ...
  const bar = (0, import_log.getBar)(`Downloading Prisma engines${hasNodeAPI ? " for Node-API" : ""} for ${(_a = options2.binaryTargets) == null ? void 0 : _a.map((p) => import_chalk.default.bold(p)).join(" and ")}`);
  const progressMap = {};
    ...
    if (await exists(sha256FilePath)) {
      ...
      if (sha256File === sha256Cache) {
        if (!targetExists) {
          ...
          await (0, import_util2.overwriteFile)(cachedFile, job.targetFilePath);
        }
...
async function getCachedBinaryPath({
  ...
  const cacheDir = await (0, import_util2.getCacheDir)(channel, version, binaryTarget);
  if (!cacheDir) {
    return null;
  }
...
function getBinaryEnvVarPath(binaryName) {
  ...
  if (envVar && process.env[envVar]) {
    ...
    if (!import_fs.default.existsSync(envVarPath)) {
      throw new Error(`Env var ${import_chalk.default.bold(envVar)} is provided but provided path ${import_chalk.default.underline(process.env[envVar])} can't be resolved.`);
    }
    debug(`Using env var ${import_chalk.default.bold(envVar)} for binary ${import_chalk.default.bold(binaryName)}, which points to ${import_chalk.default.underline(process.env[envVar])}`);
    return envVarPath;
  }
  return null;
}
...
async function downloadBinary(options2) {
  ...
  const downloadUrl = (0, import_util2.getDownloadUrl)("all_commits", version, binaryTarget, binaryName);
...
async function saveFileToCache(job, version, sha256, zippedSha256) {
  const cacheDir = await (0, import_util2.getCacheDir)(channel, version, job.binaryTarget);
  if (!cacheDir) {
    return;
  }
  ...
  try {
    await (0, import_util2.overwriteFile)(job.targetFilePath, cachedTargetPath);
